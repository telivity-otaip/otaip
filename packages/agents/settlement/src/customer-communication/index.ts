...
const VALID_CHANNELS = new Set<Channel>(['EMAIL_HTML', 'EMAIL_TEXT', 'SMS', 'WHATSAPP']);
...
export class CustomerCommunicationAgent implements Agent<
  CustomerCommunicationInput,
  CustomerCommunicationOutput
> {
  readonly id = '6.4';
  ...
  private handleGenerateNotification(
    data: CustomerCommunicationInput,
  ): CustomerCommunicationOutput {
  ...
  private validateInput(data: CustomerCommunicationInput): void {
    if (
      !data.operation ||
      !['generateNotification', 'generateBatch', 'getTemplate'].includes(data.operation)
    ) {
      throw new AgentInputValidationError(
        this.id,
        'operation',
        'Must be generateNotification, generateBatch, or getTemplate.',
      );
    }
        ...
        for (const req of data.batchRequests) {
          if (!VALID_NOTIFICATION_TYPES.has(req.notificationType)) {
            throw new AgentInputValidationError(
              this.id,
              'notificationType',
              `Invalid notification type: ${req.notificationType}`,
            );
          }
