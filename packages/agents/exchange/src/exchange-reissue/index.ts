...
export class ExchangeReissue
  implements Agent<ExchangeReissueInput, ExchangeReissueOutput>
{
  readonly id = '5.2';
  ...
  async execute(
    ...
    if (result.credit_amount !== '0.00') {
      warnings.push(`Credit of ${input.data.new_fare_currency} ${result.credit_amount} due to passenger (residual exceeds new fare).`);
    }
    if (input.data.conjunction_originals && input.data.conjunction_originals.length > 0) {
      warnings.push(`Conjunction exchange: ${input.data.conjunction_originals.length + 1} original tickets referenced.`);
    }
  ...
  private validateInput(data: ExchangeReissueInput): void {
    if (!data.original_ticket_number || !TICKET_NUMBER_RE.test(data.original_ticket_number)) {
      throw new AgentInputValidationError(this.id, 'original_ticket_number', 'Must be a 13-digit ticket number.');
    }
    ...
    if (!data.new_segments || data.new_segments.length === 0) {
      throw new AgentInputValidationError(this.id, 'new_segments', 'At least one new segment required.');
    }
    for (const seg of data.new_segments) {
      if (!CARRIER_RE.test(seg.carrier)) {
        throw new AgentInputValidationError(this.id, 'segment.carrier', `Invalid carrier: ${seg.carrier}`);
      }
      if (!AIRPORT_RE.test(seg.origin) || !AIRPORT_RE.test(seg.destination)) {
        throw new AgentInputValidationError(this.id, 'segment.origin/destination', 'Invalid airport code.');
      }
    }
    ...
    if (data.conjunction_originals) {
      for (const ct of data.conjunction_originals) {
        if (!TICKET_NUMBER_RE.test(ct)) {
          throw new AgentInputValidationError(this.id, 'conjunction_originals', `Invalid conjunction ticket number: ${ct}`);
        }
      }
    }
  }
