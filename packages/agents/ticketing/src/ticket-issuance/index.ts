...
export class TicketIssuance
  implements Agent<TicketIssuanceInput, TicketIssuanceOutput>
{
  readonly id = '4.1';
  ...
  async execute(
    ...
    if (result.is_conjunction) {
      warnings.push(`Conjunction ticketing: ${result.tickets.length} tickets generated for ${result.total_coupons} coupons.`);
    }
  ...
  private validateInput(data: TicketIssuanceInput): void {
    ...
    if (!data.passenger_name || !PASSENGER_NAME_RE.test(data.passenger_name)) {
      throw new AgentInputValidationError(this.id, 'passenger_name', 'Must be in LAST/FIRST format with alphabetic characters.');
    }
    ...
    for (const seg of data.segments) {
      ...
      if (!AIRPORT_RE.test(seg.origin) || !AIRPORT_RE.test(seg.destination)) {
        throw new AgentInputValidationError(this.id, 'segment.origin/destination', `Invalid airport code in segment.`);
      }
    }
    ...
    if (!data.form_of_payment) {
      throw new AgentInputValidationError(this.id, 'form_of_payment', 'Form of payment is required.');
    }
    if (!data.fare_calculation) {
      throw new AgentInputValidationError(this.id, 'fare_calculation', 'Fare calculation line is required.');
    }
  }
}
