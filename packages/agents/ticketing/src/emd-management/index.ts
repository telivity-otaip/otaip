...
export class EmdManagement implements Agent<EmdManagementInput, EmdManagementOutput> {
  readonly id = '4.2';
  ...
  async execute(input: AgentInput<EmdManagementInput>): Promise<AgentOutput<EmdManagementOutput>> {
    if (!this.initialized) {
    ...
    for (const svc of data.services) {
      if (!VALID_RFIC.has(svc.rfic)) {
        throw new AgentInputValidationError(
          this.id,
          'rfic',
          `Invalid RFIC code: ${svc.rfic}. Must be A-G.`,
        );
      }
      if (isNaN(Number(svc.amount))) {
        throw new AgentInputValidationError(
          this.id,
          'amount',
          'Service amount must be a valid decimal string.',
        );
      }
    }
      ...
      for (const svc of data.services) {
        if (!svc.associated_ticket_number) {
          throw new AgentInputValidationError(
            ...
            'EMD-A services must link to a ticket coupon number.',
          );
        }
        if (!TICKET_NUMBER_RE.test(svc.associated_ticket_number)) {
          throw new AgentInputValidationError(
            this.id,
            'associated_ticket_number',
            `Invalid ticket number format: ${svc.associated_ticket_number}`,
          );
        }
      }
    }
