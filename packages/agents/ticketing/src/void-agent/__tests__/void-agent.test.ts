...
describe('Void Agent', () => {
  describe('Coupon status check', () => {
    ...
    it('rejects void when coupon is Exchanged', async () => {
      const result = await agent.execute({ data: makeInput({
        coupons: [{ coupon_number: 1, status: 'O' }, { coupon_number: 2, status: 'E' }],
      }) });
  ...
  describe('Input validation', () => {
    it('rejects invalid document number', async () => {
      await expect(agent.execute({ data: makeInput({ document_number: 'INVALID' }) })).rejects.toThrow('Invalid input');
    });
    ...
    it('rejects empty coupons', async () => {
      await expect(agent.execute({ data: makeInput({ coupons: [] }) })).rejects.toThrow('Invalid input');
    });

    it('rejects missing issue datetime', async () => {
      await expect(agent.execute({ data: makeInput({ issue_datetime: '' }) })).rejects.toThrow('Invalid input');
    });
  });
