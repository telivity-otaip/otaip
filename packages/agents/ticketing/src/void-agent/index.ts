...
export class VoidAgent implements Agent<VoidAgentInput, VoidAgentOutput> {
  readonly id = '4.3';
  ...
  async execute(input: AgentInput<VoidAgentInput>): Promise<AgentOutput<VoidAgentOutput>> {
    if (!this.initialized) {
  ...
  private validateInput(data: VoidAgentInput): void {
    if (!data.document_number || !DOCUMENT_NUMBER_RE.test(data.document_number)) {
      throw new AgentInputValidationError(
        this.id,
        'document_number',
        'Must be a 13-digit ticket/EMD number.',
      );
    }
    ...
    if (
      data.settlement_system &&
      data.settlement_system !== 'BSP' &&
      data.settlement_system !== 'ARC'
    ) {
