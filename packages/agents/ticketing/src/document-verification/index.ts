...
export class DocumentVerification implements Agent<
...
> {
  ...
  async execute(
    ...
    if (result.blocking_failures > 0) {
      warnings.push(
        `${result.blocking_failures} blocking failure(s) — document issues must be resolved before travel.`,
      );
    }
  ...
  private validateInput(data: DocumentVerificationInput): void {
    ...
    if (!data.segments || data.segments.length === 0) {
      throw new AgentInputValidationError(
        this.id,
        'segments',
        'At least one travel segment required.',
      );
    }
    ...
    for (const pax of data.passengers) {
      if (!pax.ticket_name || !PASSENGER_NAME_RE.test(pax.ticket_name)) {
        throw new AgentInputValidationError(
          this.id,
          'ticket_name',
          `Invalid name format: ${pax.ticket_name ?? 'missing'}. Must be LAST/FIRST.`,
        );
      }
      ...
      if (!pax.nationality || !COUNTRY_RE.test(pax.nationality)) {
        throw new AgentInputValidationError(
          this.id,
          'nationality',
          `Invalid nationality: ${pax.nationality ?? 'missing'}. Must be ISO 2-letter.`,
        );
      }
    ...
    for (const seg of data.segments) {
      if (!seg.destination_country || !COUNTRY_RE.test(seg.destination_country)) {
        throw new AgentInputValidationError(
          this.id,
          'destination_country',
          `Invalid country code: ${seg.destination_country ?? 'missing'}`,
        );
      }
      if (!seg.travel_date) {
