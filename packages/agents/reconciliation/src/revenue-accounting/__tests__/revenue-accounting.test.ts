...
beforeAll(async () => {
  agent = new RevenueAccountingAgent();
...
async function lift(overrides: Partial<CouponLiftInput> = {}): Promise<string> {
  const r = await agent.execute({
    data: { operation: 'recordLift', coupon: makeCoupon(overrides) },
  });
  return r.data.lift!.liftId;
}

describe('RevenueAccountingAgent', () => {
  it('records a lift', async () => {
    const id = await lift();
    expect(id).toMatch(/^LFT/);
  });
  it('lift status is LIFTED', async () => {
    const r = await agent.execute({ data: { operation: 'recordLift', coupon: makeCoupon() } });
    expect(r.data.lift!.status).toBe('LIFTED');
  });
  it('rejects duplicate lift', async () => {
    await lift();
    await expect(
      agent.execute({ data: { operation: 'recordLift', coupon: makeCoupon() } }),
    ).rejects.toThrow('COUPON_ALREADY_LIFTED');
  });
  it('recognizes revenue for flight', async () => {
    await lift();
    await lift({ couponNumber: 2, fareAmount: '300.00' });
    const r = await agent.execute({
      data: { operation: 'recognizeRevenue', flightRef: 'BA115-2026-06-15' },
    });
  ...
  it('uplift report for period', async () => {
    await lift();
    const r = await agent.execute({
      data: { operation: 'getUpliftReport', period: { from: '2026-06-01', to: '2026-06-30' } },
    });
  ...
  it('deferred revenue for open coupons', async () => {
    ...
    const r = await agent.execute({
      data: { operation: 'getDeferredRevenue', currentDate: '2026-06-15' },
    });
  ...
  it('recordVoid for unknown ticket throws', async () => {
    await expect(
      agent.execute({ data: { operation: 'recordVoid', ticketNumber: 'NONEXISTENT' } }),
    ).rejects.toThrow('COUPON_NOT_FOUND');
  });
  it('recordRefund sets status REFUNDED', async () => {
    await lift();
    await agent.execute({
      data: { operation: 'recordRefund', ticketNumber: '1251234567890', refundAmount: '500.00' },
    });
    const rec = agent.getStore().get('1251234567890-1');
  ...
  it('cannot lift refunded coupon', async () => {
    ...
    await expect(
      agent.execute({ data: { operation: 'recordLift', coupon: makeCoupon() } }),
    ).rejects.toThrow('COUPON_REFUNDED');
  });
  it('empty period returns 0 in uplift', async () => {
    const r = await agent.execute({
      data: { operation: 'getUpliftReport', period: { from: '2020-01-01', to: '2020-01-31' } },
    });
  ...
  it('multiple flights recognized separately', async () => {
    ...
    const r2 = await agent.execute({
      data: { operation: 'recognizeRevenue', flightRef: 'BA200-2026-06-16' },
    });
    expect(r1.data.recognition!.couponsLifted).toBe(1);
    expect(r2.data.recognition!.couponsLifted).toBe(1);
  });
  it('has correct id', () => {
    expect(agent.id).toBe('7.6');
  });
  it('reports healthy', async () => {
    expect((await agent.health()).status).toBe('healthy');
  });
  it('throws when not initialized', async () => {
    const u = new RevenueAccountingAgent();
    await expect(u.execute({ data: { operation: 'getDeferredRevenue' } })).rejects.toThrow(
