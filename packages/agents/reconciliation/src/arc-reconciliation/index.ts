...
export class ARCReconciliation
  implements Agent<ARCReconciliationInput, ARCReconciliationOutput>
{
  readonly id = '7.2';
  ...
  async execute(
    ...
    if (result.summary.adm_dispute_expiring_count > 0) {
      warnings.push(`${result.summary.adm_dispute_expiring_count} ADM dispute window(s) expiring within 5 days.`);
    }
  ...
  private validateInput(data: ARCReconciliationInput): void {
    ...
    if (!data.iar_records || !Array.isArray(data.iar_records)) {
      throw new AgentInputValidationError(this.id, 'iar_records', 'Must be an array of IAR records.');
    }
    ...
    for (const iar of data.iar_records) {
      if (!iar.document_number || !DOCUMENT_NUMBER_RE.test(iar.document_number)) {
        throw new AgentInputValidationError(this.id, 'iar_records.document_number', `Invalid document number: ${iar.document_number ?? 'missing'}`);
      }
    }
  }
}
