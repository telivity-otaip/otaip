...
beforeAll(async () => {
  agent = new CorporatePolicyValidationAgent();
...
function makeInput(overrides: Partial<PolicyValidationInput['offer']> = {}): PolicyValidationInput {
  return {
    offer: {
      ...
      segments: [{ origin: 'LHR', destination: 'JFK', durationMinutes: 480 }],
      ...overrides,
    },
    policy: POLICY,
  };
...
describe('CorporatePolicyValidationAgent', () => {
  ...
  it('detects cabin violation (F on intl short-haul)', async () => {
    const r = await agent.execute({
      data: makeInput({
        cabin: 'F',
        segments: [{ origin: 'LHR', destination: 'CDG', durationMinutes: 90 }],
      }),
    });
  ...
  it('hard violation for advance booking under threshold', async () => {
    ...
    expect(
      r.data.violations.some((v) => v.rule === 'ADVANCE_BOOKING' && v.severity === 'HARD'),
    ).toBe(true);
  });
  it('bypass code converts soft to approved', async () => {
    const r = await agent.execute({
      data: { ...makeInput({ advanceBookingDays: 7 }), bypassCode: 'BYPASS123' },
    });
    expect(r.data.result).toBe('APPROVED');
  ...
  it('bypass does not override hard violation', async () => {
    const r = await agent.execute({
      data: { ...makeInput({ carrier: 'FR' }), bypassCode: 'BYPASS123' },
    });
    expect(r.data.result).toBe('HARD_VIOLATION');
  });
  it('invalid bypass code not applied', async () => {
    const r = await agent.execute({
      data: { ...makeInput({ advanceBookingDays: 7 }), bypassCode: 'WRONG' },
    });
  ...
  it('multiple violations returned', async () => {
    const r = await agent.execute({
      data: makeInput({ cabin: 'F', fareAmount: '6000.00', carrier: 'FR' }),
    });
  ...
  it('no fare ceiling rule if not set', async () => {
    const r = await agent.execute({
      data: { offer: makeInput().offer, policy: { ...POLICY, fareRules: { blockedCarriers: [] } } },
    });
    expect(r.data.violations.some((v) => v.rule === 'FARE_CEILING')).toBe(false);
  });
  ...
  it('3 days advance exactly passes hard check', async () => {
    ...
    expect(
      r.data.violations.some((v) => v.rule === 'ADVANCE_BOOKING' && v.severity === 'SOFT'),
    ).toBe(true);
  });
});
