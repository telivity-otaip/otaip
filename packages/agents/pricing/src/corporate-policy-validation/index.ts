...
export class CorporatePolicyValidationAgent implements Agent<
  PolicyValidationInput,
  PolicyValidationOutput
> {
  readonly id = '2.5';
  readonly name = 'Corporate Policy Validation';
  ...
  async execute(
    input: AgentInput<PolicyValidationInput>,
  ): Promise<AgentOutput<PolicyValidationOutput>> {
    ...
    // Cabin check
    const isLongHaul = offer.segments.some(
      (s) => s.durationMinutes >= policy.cabinRules.longHaulThresholdMinutes,
    );
    ...
    if (CABIN_ORDER[offer.cabin] > CABIN_ORDER[maxCabin]) {
      violations.push({
        rule: 'CABIN_CLASS',
        severity: 'HARD',
        detail: `Cabin ${offer.cabin} exceeds max ${maxCabin}.`,
        policyValue: maxCabin,
        actualValue: offer.cabin,
      });
    }
    ...
    if (policy.fareRules.maxFareAmount) {
      ...
      if (fare.greaterThan(max)) {
        violations.push({
          rule: 'FARE_CEILING',
          severity: 'HARD',
          detail: `Fare ${offer.fareAmount} exceeds max ${policy.fareRules.maxFareAmount}.`,
          policyValue: policy.fareRules.maxFareAmount,
          actualValue: offer.fareAmount,
        });
      }
    }
    ...
    if (policy.fareRules.blockedCarriers?.includes(offer.carrier)) {
      violations.push({
        rule: 'BLOCKED_CARRIER',
        severity: 'HARD',
        detail: `Carrier ${offer.carrier} is blocked.`,
        policyValue: 'blocked',
        actualValue: offer.carrier,
      });
    }

    // Advance booking
    if (
      policy.bookingRules.minAdvanceDaysHard !== undefined &&
      offer.advanceBookingDays < policy.bookingRules.minAdvanceDaysHard
    ) {
      violations.push({
        rule: 'ADVANCE_BOOKING',
        severity: 'HARD',
        detail: `${offer.advanceBookingDays} days advance (hard min: ${policy.bookingRules.minAdvanceDaysHard}).`,
        policyValue: String(policy.bookingRules.minAdvanceDaysHard),
        actualValue: String(offer.advanceBookingDays),
      });
    } else if (
      policy.bookingRules.minAdvanceDays !== undefined &&
      offer.advanceBookingDays < policy.bookingRules.minAdvanceDays
    ) {
      violations.push({
        rule: 'ADVANCE_BOOKING',
        severity: 'SOFT',
        detail: `${offer.advanceBookingDays} days advance (soft min: ${policy.bookingRules.minAdvanceDays}).`,
        policyValue: String(policy.bookingRules.minAdvanceDays),
        actualValue: String(offer.advanceBookingDays),
      });
    }
