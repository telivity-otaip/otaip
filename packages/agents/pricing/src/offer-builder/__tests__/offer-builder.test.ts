...
describe('OfferBuilderAgent', () => {
  it('builds offer with correct subtotal', async () => {
    const r = await agent.execute({
      data: { operation: 'buildOffer', buildInput: BUILD, currentTime: '2026-04-01T12:00:00Z' },
    });
    ...
    const r = await agent.execute({
      data: {
        ...
        buildInput: {
          ...BUILD,
          ancillaries: [
            { ancillaryId: 'A1', amount: '50.00', currency: 'USD', description: 'Bag' },
          ],
        },
        currentTime: '2026-04-01T12:00:00Z',
      },
  ...
  it('NDC TTL = 15min', async () => {
    const r = await agent.execute({
      data: {
        operation: 'buildOffer',
        buildInput: { ...BUILD, pricingSource: 'NDC' as const },
        currentTime: '2026-04-01T12:00:00Z',
      },
    });
    const exp = new Date(r.data.offer!.expiresAt);
  ...
  it('custom TTL', async () => {
    const r = await agent.execute({
      data: {
        operation: 'buildOffer',
        buildInput: { ...BUILD, ttlMinutes: 60 },
        currentTime: '2026-04-01T12:00:00Z',
      },
    });
    const exp = new Date(r.data.offer!.expiresAt);
  ...
  it('markUsed twice throws ALREADY_USED', async () => {
    ...
    await expect(
      agent.execute({
        data: { operation: 'markUsed', offerId: id, currentTime: '2026-04-01T12:06:00Z' },
      }),
    ).rejects.toThrow('OFFER_ALREADY_USED');
  });
  ...
  it('rejects invalid fare amount', async () => {
    await expect(
      agent.execute({
        data: {
          operation: 'buildOffer',
          buildInput: { ...BUILD, fare: { ...BUILD.fare, baseAmount: 'abc' } },
        },
      }),
    ).rejects.toThrow('INVALID_FARE_AMOUNT');
  });
  ...
  it('DIRECT TTL = 20min', async () => {
    const r = await agent.execute({
      data: {
        operation: 'buildOffer',
        buildInput: { ...BUILD, pricingSource: 'DIRECT' as const },
        currentTime: '2026-04-01T12:00:00Z',
      },
    });
    const exp = new Date(r.data.offer!.expiresAt);
  ...
  it('throws when not initialized', async () => {
    ...
    await expect(u.execute({ data: { operation: 'getOffer', offerId: 'X' } })).rejects.toThrow(
  ...
  it('validates used offer returns ALREADY_USED', async () => {
    ...
    const r = await agent.execute({
      data: { operation: 'validateOffer', offerId: id, currentTime: '2026-04-01T12:06:00Z' },
    });
    expect(r.data.valid).toBe(false);
  ...
  it('validateOffer not found', async () => {
    const r = await agent.execute({
      data: { operation: 'validateOffer', offerId: 'NONE', currentTime: '2026-04-01T12:00:00Z' },
    });
    expect(r.data.valid).toBe(false);
