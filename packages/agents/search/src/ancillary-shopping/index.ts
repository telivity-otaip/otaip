...
export interface AncillaryAdapter {
  name: string;
  searchAncillaries(input: AncillaryShoppingInput): Promise<AncillaryOffer[]>;
}

export class AncillaryShoppingAgent implements Agent<
...
> {
  ...
  setAdapter(adapter: AncillaryAdapter): void {
  ...
  async execute(
    input: AgentInput<AncillaryShoppingInput>,
  ): Promise<AgentOutput<AncillaryShoppingOutput>> {
    ...
    if (this.adapter) {
      ...
      return {
        data: {
          ancillaries,
          notSupportedByAdapter: false,
          currency: ancillaries[0]?.price.currency ?? 'USD',
        },
        confidence: 1.0,
    ...
    return {
      data: { ancillaries: [], notSupportedByAdapter: true, currency: 'USD' },
      confidence: 0,
      metadata: { agent_id: this.id },
    };
  }
