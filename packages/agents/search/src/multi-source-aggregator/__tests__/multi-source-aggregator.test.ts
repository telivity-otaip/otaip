...
beforeAll(async () => { agent = new MultiSourceAggregatorAgent(); await agent.initialize(); });
afterAll(() => { agent.destroy(); });

function makeFlight(overrides: Partial<SearchResult> = {}): SearchResult {
  return { carrier: 'BA', flightNumber: '115', origin: 'LHR', destination: 'JFK', departureDate: '2026-06-15', departureTime: '09:00', arrivalTime: '12:00', durationMinutes: 480, stops: 0, price: { amount: '500.00', currency: 'USD' }, cabin: 'Y', ...overrides };
}
...
const MULTI_INPUT: MultiSourceInput = {
  results: [
    makeAdapter('GDS', [makeFlight({ price: { amount: '500.00', currency: 'USD' } }), makeFlight({ flightNumber: '200', price: { amount: '600.00', currency: 'USD' } })]),
...
describe('MultiSourceAggregatorAgent', () => {
  ...
  it('keep_first keeps first encountered', async () => {
    const r = await agent.execute({ data: { ...MULTI_INPUT, deduplicationStrategy: 'keep_first' } });
    const ba115 = r.data.flights.find((f) => f.flightNumber === '115');
  ...
  it('ranks by price ascending', async () => {
    ...
    for (let i = 1; i < r.data.flights.length; i++) {
      expect(Number(r.data.flights[i]!.lowestPrice.amount)).toBeGreaterThanOrEqual(Number(r.data.flights[i - 1]!.lowestPrice.amount));
    }
  });
  it('ranks by duration', async () => {
    const input: MultiSourceInput = {
      results: [makeAdapter('GDS', [makeFlight({ durationMinutes: 600 }), makeFlight({ flightNumber: '200', durationMinutes: 300 })])],
      deduplicationStrategy: 'keep_cheapest', rankBy: 'duration',
    };
  ...
  it('ranks by stops', async () => {
    const input: MultiSourceInput = {
      results: [makeAdapter('GDS', [makeFlight({ stops: 2 }), makeFlight({ flightNumber: '200', stops: 0 })])],
      deduplicationStrategy: 'keep_cheapest', rankBy: 'stops',
    };
    const r = await agent.execute({ data: input });
  ...
  it('partial failure: errored adapter included in summary', async () => {
    const input: MultiSourceInput = {
      ...
      deduplicationStrategy: 'keep_cheapest', rankBy: 'price',
    };
    const r = await agent.execute({ data: input });
    expect(r.data.flights.length).toBe(1);
  ...
  it('empty adapters return empty flights', async () => {
    const r = await agent.execute({ data: { results: [], deduplicationStrategy: 'keep_cheapest', rankBy: 'price' } });
  ...
  it('has correct agent id', () => { expect(agent.id).toBe('1.6'); });
  ...
  it('single adapter single flight works', async () => {
    const input: MultiSourceInput = { results: [makeAdapter('GDS', [makeFlight()])], deduplicationStrategy: 'keep_cheapest', rankBy: 'price' };
    const r = await agent.execute({ data: input });
    expect(r.data.flights.length).toBe(1);
  });
  it('different routes are not deduped', async () => {
    const input: MultiSourceInput = {
      results: [makeAdapter('GDS', [makeFlight(), makeFlight({ origin: 'CDG', destination: 'LAX' })])],
      deduplicationStrategy: 'keep_cheapest', rankBy: 'price',
    };
    const r = await agent.execute({ data: input });
