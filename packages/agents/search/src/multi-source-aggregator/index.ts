...
import type { MultiSourceInput, MultiSourceOutput, NormalizedFlight, SearchResult } from './types.js';
...
export class MultiSourceAggregatorAgent implements Agent<MultiSourceInput, MultiSourceOutput> {
  ...
  async execute(input: AgentInput<MultiSourceInput>): Promise<AgentOutput<MultiSourceOutput>> {
    ...
    let totalRaw = 0;
    const grouped = new Map<string, { flights: Array<{ adapter: string; flight: SearchResult }> }>();
    ...
    for (const [, group] of grouped) {
      const allPrices = group.flights.map((g) => ({ adapter: g.adapter, amount: g.flight.price.amount, currency: g.flight.price.currency }));
      const cheapest = allPrices.reduce((a, b) => new Decimal(a.amount).lessThan(new Decimal(b.amount)) ? a : b);

      if (d.deduplicationStrategy === 'keep_cheapest') {
        const cheapestEntry = group.flights.reduce((a, b) => new Decimal(a.flight.price.amount).lessThan(new Decimal(b.flight.price.amount)) ? a : b);
        flights.push({
    ...
    // Sort
    if (d.rankBy === 'price') flights.sort((a, b) => new Decimal(a.lowestPrice.amount).comparedTo(new Decimal(b.lowestPrice.amount)));
    ...
    const adapterSummary = d.results.map((r) => ({
      adapter: r.adapterName, count: r.results.length, error: r.error, responseTimeMs: r.responseTimeMs,
    }));

    return {
      data: { flights, totalRaw, totalAfterDedup: flights.length, adapterSummary, rankBy: d.rankBy },
      confidence: 1.0,
...
export type { MultiSourceInput, MultiSourceOutput, NormalizedFlight, SearchResult, AdapterSearchResult, AdapterSummary, DeduplicationStrategy, RankBy } from './types.js';
