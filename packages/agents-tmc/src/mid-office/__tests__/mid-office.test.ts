...
function makeSeg(overrides: Partial<PnrSegment> = {}): PnrSegment {
  return {
    ...
    origin_country: 'GB', destination_country: 'US',
    departure_date: '2026-06-15', departure_time: '09:00',
    status: 'HK', booking_class: 'Y',
    ...overrides,
  };
}
...
function makePnr(overrides: Partial<MockPnr> = {}): MockPnr {
  return {
    recloc: 'ABC123', passenger_name: 'SMITH/JOHN',
    segments: [makeSeg()],
    ticket_deadline: '2026-06-10T12:00:00Z',
    apis_complete: true, contact_present: true, fop_present: true,
    ...overrides,
  };
}
...
describe('Mid-Office Automation', () => {
  describe('TTL check', () => {
    ...
    it('urgent when deadline within 1 hour', async () => {
      ...
      expect(res.data.results[0]!.issues.some((i) => i.code === 'TTL_URGENT' && i.severity === 'urgent')).toBe(true);
    });
  ...
  describe('PNR completeness', () => {
    ...
    it('no flags when complete', async () => {
      const res = await agent.execute({ data: makeInput() });
      expect(res.data.results[0]!.issues.filter((i) => i.code === 'MISSING_APIS' || i.code === 'MISSING_CONTACT' || i.code === 'MISSING_FOP')).toHaveLength(0);
    });
  });
    ...
    it('flags domestic business with corporate_id', async () => {
      const res = await agent.execute({ data: makeInput({
        pnrs: [makePnr({
          ...
          segments: [makeSeg({ origin_country: 'US', destination_country: 'US', cabin: 'business' })],
        })],
      }) });
  ...
  describe('Married segments', () => {
    it('flags incomplete married group', async () => {
      ...
      expect(res.data.results[0]!.issues.some((i) => i.code === 'MARRIED_SEGMENT_INCOMPLETE')).toBe(true);
    });

    it('no flag when married group has 2+ segments', async () => {
      const res = await agent.execute({ data: makeInput({
        pnrs: [makePnr({ segments: [
          makeSeg({ married_group: 'M1' }),
          makeSeg({ flight_number: '116', origin: 'JFK', destination: 'LAX', married_group: 'M1' }),
        ] })],
      }) });
      expect(res.data.results[0]!.issues.some((i) => i.code === 'MARRIED_SEGMENT_INCOMPLETE')).toBe(false);
    });
  });
  ...
  describe('agent compliance', () => {
    it('has correct id/name', () => { expect(agent.id).toBe('8.3'); expect(agent.name).toBe('Mid-Office Automation'); });
