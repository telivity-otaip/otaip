...
beforeEach(() => { agent.getAccounts().clear(); });
...
function createAccount(): CorporateAccountInput {
  return {
    ...
    account_data: {
      company_name: 'Acme Corp', policy: DEFAULT_POLICY,
      negotiated_fares: [{ airline: 'BA', fare_basis: 'YBAACME', cabin: 'economy', discount_percent: 15, valid_from: '2026-01-01', valid_to: '2026-12-31' }],
      contact_email: 'travel@acme.com', contact_name: 'Travel Manager',
    },
...
describe('Corporate Account', () => {
  describe('create / get / update / list', () => {
    ...
    it('throws ACCOUNT_NOT_FOUND', async () => {
      await expect(agent.execute({ data: { operation: 'get_account', account_id: 'NONE' } })).rejects.toThrow('ACCOUNT_NOT_FOUND');
    });

    it('updates account', async () => {
      const id = await createAndGetId();
      const res = await agent.execute({ data: { operation: 'update_account', account_id: id, account_data: { contact_name: 'New Manager' }, current_date: '2026-04-02' } });
    ...
    it('gets preferred suppliers', async () => {
      const id = await createAndGetId();
      const res = await agent.execute({ data: { operation: 'get_preferred_suppliers', account_id: id } });
  ...
  describe('validate_booking', () => {
    it('in-policy booking passes', async () => {
      ...
      const res = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'BA', origin: 'LHR', destination: 'JFK', origin_country: 'GB', destination_country: 'US', departure_date: '2026-05-15', cabin: 'business', flight_duration_hours: 8 }], fare_amount_usd: '2500.00', airline: 'BA' },
    ...
    it('detects cabin violation (domestic business)', async () => {
      ...
      const res = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'AA', origin: 'JFK', destination: 'LAX', origin_country: 'US', destination_country: 'US', departure_date: '2026-05-15', cabin: 'business', flight_duration_hours: 5 }], fare_amount_usd: '800.00', airline: 'AA' },
        current_date: '2026-04-01',
      } });
      expect(res.data.validation!.violations.some((v) => v.rule === 'cabin_class' && v.severity === 'hard')).toBe(true);
    });
    ...
    it('detects advance booking soft violation', async () => {
      ...
      const res = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'BA', origin: 'LHR', destination: 'JFK', origin_country: 'GB', destination_country: 'US', departure_date: '2026-04-08', cabin: 'economy', flight_duration_hours: 8 }], fare_amount_usd: '500.00', airline: 'BA' },
        current_date: '2026-04-01',
      } });
      expect(res.data.validation!.violations.some((v) => v.rule === 'advance_booking' && v.severity === 'soft')).toBe(true);
    });
    ...
    it('detects advance booking hard violation (under exception threshold)', async () => {
      ...
      const res = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'BA', origin: 'LHR', destination: 'JFK', origin_country: 'GB', destination_country: 'US', departure_date: '2026-04-02', cabin: 'economy', flight_duration_hours: 8 }], fare_amount_usd: '500.00', airline: 'BA' },
        current_date: '2026-04-01',
      } });
      expect(res.data.validation!.violations.some((v) => v.rule === 'advance_booking' && v.severity === 'hard')).toBe(true);
    });
    ...
    it('requires approval above threshold', async () => {
      ...
      const res = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'BA', origin: 'LHR', destination: 'JFK', origin_country: 'GB', destination_country: 'US', departure_date: '2026-05-15', cabin: 'business', flight_duration_hours: 8 }], fare_amount_usd: '4000.00', airline: 'BA' },
        current_date: '2026-04-01',
      } });
    ...
    it('includes negotiated fare when available', async () => {
      ...
      const res = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'BA', origin: 'LHR', destination: 'JFK', origin_country: 'GB', destination_country: 'US', departure_date: '2026-05-15', cabin: 'economy', flight_duration_hours: 8 }], fare_amount_usd: '1000.00', airline: 'BA' },
        current_date: '2026-04-01',
      } });
    ...
    it('blocks when out_of_policy_booking_allowed=false with hard violation', async () => {
      ...
      const valRes = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'AA', origin: 'JFK', destination: 'LAX', origin_country: 'US', destination_country: 'US', departure_date: '2026-05-15', cabin: 'business', flight_duration_hours: 5 }], fare_amount_usd: '800.00', airline: 'AA' },
        current_date: '2026-04-01',
      } });
    ...
    it('rejects blacklisted airline', async () => {
      ...
      await expect(agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'FR', origin: 'DUB', destination: 'STN', origin_country: 'IE', destination_country: 'GB', departure_date: '2026-05-15', cabin: 'economy', flight_duration_hours: 1.5 }], fare_amount_usd: '50.00', airline: 'FR' },
        current_date: '2026-04-01',
    ...
    it('international under 6h uses correct policy', async () => {
      ...
      const res = await agent.execute({ data: {
        operation: 'validate_booking', account_id: id,
        booking: { segments: [{ carrier: 'BA', origin: 'LHR', destination: 'CDG', origin_country: 'GB', destination_country: 'FR', departure_date: '2026-05-15', cabin: 'business', flight_duration_hours: 1.5 }], fare_amount_usd: '500.00', airline: 'BA' },
        current_date: '2026-04-01',
      } });
  ...
  describe('agent compliance', () => {
    it('has correct id/name', () => { expect(agent.id).toBe('8.2'); });
    it('reports healthy', async () => { expect((await agent.health()).status).toBe('healthy'); });
    it('throws when not initialized', async () => {
      const u = new CorporateAccountAgent();
      await expect(u.execute({ data: { operation: 'list_accounts' } })).rejects.toThrow('not been initialized');
    });
  });
});
