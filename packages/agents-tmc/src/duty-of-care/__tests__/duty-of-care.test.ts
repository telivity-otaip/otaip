...
describe('Duty of Care', () => {
  ...
  describe('get_traveler_itinerary', () => {
    it('returns itinerary', async () => {
      const res = await agent.execute({
        data: { operation: 'get_traveler_itinerary', traveler_id: 'TVL001' },
      });
    ...
    it('throws for unknown traveler', async () => {
      await expect(
        agent.execute({ data: { operation: 'get_traveler_itinerary', traveler_id: 'NONE' } }),
  ...
  describe('assess_destination_risk', () => {
    it('returns low risk for US', async () => {
      const res = await agent.execute({
        data: { operation: 'assess_destination_risk', destination_country: 'US' },
      });
    ...
    it('returns medium risk for Mexico', async () => {
      const res = await agent.execute({
        data: { operation: 'assess_destination_risk', destination_country: 'MX' },
      });
    ...
    it('returns critical for Afghanistan', async () => {
      const res = await agent.execute({
        data: { operation: 'assess_destination_risk', destination_country: 'AF' },
      });
    ...
    it('returns medium for unknown country', async () => {
      const res = await agent.execute({
        data: { operation: 'assess_destination_risk', destination_country: 'XX' },
      });
      expect(res.data.risk!.risk_level).toBe('medium');
    });
    ...
    it('rejects invalid country code', async () => {
      await expect(
        agent.execute({
          data: { operation: 'assess_destination_risk', destination_country: 'INVALID' },
        }),
  ...
  describe('mark_accounted_for', () => {
    ...
    it('is idempotent', async () => {
      await agent.execute({
        data: { operation: 'mark_accounted_for', traveler_id: 'TVL001', incident_id: 'INC001' },
      });
      const res = await agent.execute({
        data: { operation: 'mark_accounted_for', traveler_id: 'TVL001', incident_id: 'INC001' },
      });
  ...
  describe('agent compliance', () => {
    it('has correct id/name', () => {
      expect(agent.id).toBe('8.5');
      expect(agent.name).toBe('Duty of Care');
    });
    ...
    it('throws when not initialized', async () => {
      ...
      await expect(
        u.execute({ data: { operation: 'locate_travelers', date: '2026-06-15' } }),
