...
const SEED_PLUGINS: Omit<Plugin, 'registered_at'>[] = [
  {
    plugin_id: 'duffel-adapter', name: 'Duffel NDC Adapter', version: '1.0.0',
    description: 'Duffel API distribution adapter for NDC content.',
    author: 'OTAIP', capabilities: ['availability_search', 'booking', 'ticketing'],
    agent_ids: ['1.1', '3.1'], enabled: true,
  },
  {
    plugin_id: 'amadeus-adapter', name: 'Amadeus GDS Adapter', version: '2.1.0',
    description: 'Amadeus GDS adapter for legacy distribution.',
    author: 'OTAIP', capabilities: ['availability_search', 'booking', 'ticketing', 'queues'],
    agent_ids: ['1.1', '3.1', '3.2', '3.4'], enabled: true,
  },
  {
    plugin_id: 'expense-reporter', name: 'Corporate Expense Reporter', version: '0.5.0',
    description: 'Generates expense reports from travel bookings.',
    author: 'Third Party', capabilities: ['reporting', 'expense_management'],
    agent_ids: ['8.4'], enabled: false,
  },
];

export class PluginManagerAgent
  implements Agent<PluginInput, PluginOutput>
{
  readonly id = '9.5';
  ...
  async execute(
    input: AgentInput<PluginInput>,
  ): Promise<AgentOutput<PluginOutput>> {
    if (!this.initialized) throw new AgentNotInitializedError(this.id);

    const d = input.data;

    switch (d.operation) {
      case 'register_plugin': return this.handleRegister(d);
      case 'unregister_plugin': return this.handleUnregister(d);
      case 'list_plugins': return this.handleList();
      case 'get_plugin': return this.handleGet(d);
      case 'discover_capabilities': return this.handleDiscover(d);
      case 'enable_plugin': return this.handleToggle(d, true);
      case 'disable_plugin': return this.handleToggle(d, false);
      default:
  ...
  private handleRegister(d: PluginInput): AgentOutput<PluginOutput> {
    if (!d.plugin_data) throw new AgentInputValidationError(this.id, 'plugin_data', 'MISSING_REQUIRED_FIELD');
    const pd = d.plugin_data;

    if (!pd.plugin_id) throw new AgentInputValidationError(this.id, 'plugin_id', 'MISSING_REQUIRED_FIELD');
    if (!pd.name || pd.name.trim().length === 0) throw new AgentInputValidationError(this.id, 'name', 'MISSING_REQUIRED_FIELD');
    if (!pd.version || !SEMVER_RE.test(pd.version)) throw new AgentInputValidationError(this.id, 'version', 'INVALID_VERSION_FORMAT');
    if (!pd.capabilities || pd.capabilities.length === 0) throw new AgentInputValidationError(this.id, 'capabilities', 'MISSING_REQUIRED_FIELD');
    if (!pd.agent_ids || pd.agent_ids.length === 0) throw new AgentInputValidationError(this.id, 'agent_ids', 'MISSING_REQUIRED_FIELD');
  ...
  private handleUnregister(d: PluginInput): AgentOutput<PluginOutput> {
    if (!d.plugin_id) throw new AgentInputValidationError(this.id, 'plugin_id', 'MISSING_REQUIRED_FIELD');
    if (!this.plugins.has(d.plugin_id)) throw new AgentInputValidationError(this.id, 'plugin_id', 'PLUGIN_NOT_FOUND');
