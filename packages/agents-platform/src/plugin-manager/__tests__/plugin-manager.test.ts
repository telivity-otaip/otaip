...
describe('Plugin Manager', () => {
  ...
  describe('get_plugin', () => {
    it('gets plugin by ID', async () => {
      const res = await agent.execute({
        data: { operation: 'get_plugin', plugin_id: 'duffel-adapter' },
      });
    ...
    it('throws PLUGIN_NOT_FOUND', async () => {
      await expect(
        agent.execute({ data: { operation: 'get_plugin', plugin_id: 'nonexistent' } }),
  ...
  describe('unregister_plugin', () => {
    it('removes plugin', async () => {
      ...
      await expect(
        agent.execute({ data: { operation: 'get_plugin', plugin_id: 'to-remove' } }),
      ).rejects.toThrow('PLUGIN_NOT_FOUND');
    });
  });
  ...
  describe('enable/disable', () => {
    it('disables plugin', async () => {
      const res = await agent.execute({
        data: { operation: 'disable_plugin', plugin_id: 'amadeus-adapter' },
      });
    ...
    it('enables plugin', async () => {
      ...
      const res = await agent.execute({
        data: { operation: 'enable_plugin', plugin_id: 'amadeus-adapter' },
      });
      expect(res.data.plugin!.enabled).toBe(true);
    });
    ...
    it('enable is idempotent', async () => {
      const res = await agent.execute({
        data: { operation: 'enable_plugin', plugin_id: 'duffel-adapter' },
      });
      expect(res.data.plugin!.enabled).toBe(true);
    });
  });
  ...
  describe('discover_capabilities', () => {
    ...
    it('filters by capability', async () => {
      const res = await agent.execute({
        data: { operation: 'discover_capabilities', capability: 'booking' },
      });
    ...
    it('excludes disabled plugins', async () => {
      ...
      const res = await agent.execute({
        data: { operation: 'discover_capabilities', capability: 'expense_management' },
      });
  ...
  describe('Agent compliance', () => {
    ...
    it('throws when not initialized', async () => {
      ...
      await expect(u.execute({ data: { operation: 'list_plugins' } })).rejects.toThrow(
