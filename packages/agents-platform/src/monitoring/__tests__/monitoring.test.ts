...
async function recordMetrics(agentId: string, successes: number, errors: number, latencies: number[]): Promise<void> {
  const ts = '2026-04-01T12:00:00Z';
  for (let i = 0; i < successes; i++) {
    await agent.execute({ data: { operation: 'record_metric', agent_id: agentId, metric_type: 'success', timestamp: ts } });
  }
  for (let i = 0; i < errors; i++) {
    await agent.execute({ data: { operation: 'record_metric', agent_id: agentId, metric_type: 'error', timestamp: ts } });
  }
  for (const lat of latencies) {
    await agent.execute({ data: { operation: 'record_metric', agent_id: agentId, metric_type: 'latency_ms', value: lat, timestamp: ts } });
  }
}
...
describe('Monitoring & Alerting', () => {
  describe('record_metric', () => {
    it('records a metric', async () => {
      const res = await agent.execute({ data: { operation: 'record_metric', agent_id: '1.1', metric_type: 'success', timestamp: '2026-04-01T12:00:00Z' } });
      expect(res.data.message).toBe('Metric recorded.');
    });

    it('rejects missing agent_id', async () => {
      await expect(agent.execute({ data: { operation: 'record_metric', metric_type: 'success' } })).rejects.toThrow('Invalid');
    });

    it('rejects missing metric_type', async () => {
      await expect(agent.execute({ data: { operation: 'record_metric', agent_id: '1.1' } })).rejects.toThrow('Invalid');
    });
  });
  ...
  describe('get_health', () => {
    it('returns healthy for low error rate and latency', async () => {
      ...
      const res = await agent.execute({ data: { operation: 'get_health', current_datetime: '2026-04-01T12:01:00Z' } });
  ...
  describe('Alerts', () => {
    it('fires alert when agent becomes degraded', async () => {
      ...
      await agent.execute({ data: { operation: 'get_health', current_datetime: '2026-04-01T12:01:00Z' } });
      const res = await agent.execute({ data: { operation: 'list_alerts' } });
      expect(res.data.alerts!.some((a) => a.agent_id === 'A1' && a.severity === 'warning')).toBe(true);
    });
    ...
    it('fires critical alert when agent is down', async () => {
      ...
      await agent.execute({ data: { operation: 'get_health', current_datetime: '2026-04-01T12:01:00Z' } });
      const res = await agent.execute({ data: { operation: 'list_alerts' } });
      expect(res.data.alerts!.some((a) => a.agent_id === 'A2' && a.severity === 'critical')).toBe(true);
    });

    it('does not fire duplicate alerts for same agent', async () => {
      await recordMetrics('A3', 8, 2, [100]);
      await agent.execute({ data: { operation: 'get_health', current_datetime: '2026-04-01T12:01:00Z' } });
      await agent.execute({ data: { operation: 'get_health', current_datetime: '2026-04-01T12:02:00Z' } });
      const res = await agent.execute({ data: { operation: 'list_alerts' } });
    ...
    it('acknowledge is idempotent', async () => {
      ...
      await agent.execute({ data: { operation: 'acknowledge_alert', alert_id: alert!.alert_id } });
      const res = await agent.execute({ data: { operation: 'acknowledge_alert', alert_id: alert!.alert_id } });
  ...
  describe('SLA report', () => {
    it('generates SLA report for period', async () => {
      ...
      const res = await agent.execute({ data: { operation: 'get_sla_report', date_from: '2026-04-01T00:00:00Z', date_to: '2026-04-02T00:00:00Z' } });
    ...
    it('filters by agent_ids', async () => {
      ...
      const res = await agent.execute({ data: { operation: 'get_sla_report', date_from: '2026-04-01T00:00:00Z', date_to: '2026-04-02T00:00:00Z', agent_ids: ['S2'] } });
      expect(res.data.sla_report!.every((r) => r.agent_id === 'S2')).toBe(true);
    });

    it('rejects missing date range', async () => {
      await expect(agent.execute({ data: { operation: 'get_sla_report' } })).rejects.toThrow('Invalid');
    });
  });

  describe('Agent compliance', () => {
    it('has correct id/name', () => { expect(agent.id).toBe('9.3'); });
    it('reports healthy', async () => { expect((await agent.health()).status).toBe('healthy'); });
    it('throws when not initialized', async () => {
      const u = new MonitoringAgent();
      await expect(u.execute({ data: { operation: 'list_alerts' } })).rejects.toThrow('not been initialized');
    });
  });
});
