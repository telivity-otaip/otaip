/**
 * OTAIP Sabre Booking Demo
 *
 * Claude acts as an intelligent booking agent using the Sabre GDS adapter
 * from Telivity Connect. It searches flights via Bargain Finder Max v5,
 * evaluates options, prices the best one, and creates a HOLD booking
 * via the Booking Management API.
 *
 * Flow: Claude (tool_use) -> OTAIP SabreAdapter -> Sabre REST APIs
 *
 ...
 *
 * Run: pnpm --filter @otaip/demo book:sabre
 */
...
const tools: Anthropic.Tool[] = [
  {
    name: 'search_flights',
    description:
      'Search for available flights via Sabre Bargain Finder Max. Returns all offers with full itinerary and price details. Evaluate ALL options and select the best one based on the traveler context.',
    input_schema: {
  ...
  },
  {
    name: 'price_offer',
    description:
      'Confirm current price and availability for the selected offer via Sabre revalidation. Always call this immediately before booking. If unavailable, pick the next best option.',
    input_schema: {
      type: 'object' as const,
      properties: {
  ...
  },
  {
    name: 'book_flight',
    description:
      'Create a HOLD booking via Sabre Booking Management API. Returns a PNR/confirmation ID. Only call this after pricing confirms availability.',
    input_schema: {
      type: 'object' as const,
      properties: {
  ...
  try {
    ...
    if (name === 'book_flight') {
      ...
      if (result.paymentDeadline) {
        console.log(`Payment deadline: ${result.paymentDeadline}`);
      }
      ...
      return JSON.stringify({
        ...
        payment_deadline: result.paymentDeadline ?? null,
      });
    }
...
async function run() {
  ...
  const userMessage =
    ...
    `Create a HOLD booking and tell me what you chose and why.`;

  const systemPrompt =
    `You are an intelligent travel booking agent powered by OTAIP and Sabre GDS. ` +
  ...
  console.log('='.repeat(60));
  console.log('OTAIP Sabre Booking Demo - Intelligent Agent');
  console.log('='.repeat(60));
